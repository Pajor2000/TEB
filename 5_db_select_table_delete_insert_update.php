      ...
      while ($row = $result->fetch_assoc()) {
        echo <<< USER
          <tr>
            ...
            <td>$row[surname]</td>
            <td>$row[city]</td>
            <td>$row[birthday]</td><td><a href="./scripts/delete.php?deleteId=$row[user_id]">Usuń</a></td>
...
$userUpdate = $result->fetch_assoc();
  ...
  echo <<< FORMADDUSER
    ...
    <form action="./scripts/insert.php" method="post">
      <input type="text" name="name" value=$userUpdate[name] placeholder="Podaj imię"><br><br>
      <input type="text" name="surname" value=$userUpdate[surname] placeholder="Podaj nazwisko"><br><br>
      <select name="city_id">
FORMADDUSER;
$sql="SELECT * FROM `cities`";
$result=$connect->query($sql);
